...
async function findAll() {
  return (await db.query(`${SELECT_POST} ORDER BY p.created_at DESC`)).rows;
}

async function findById(id) {
  const { rows } = await db.query(`${SELECT_POST} WHERE p.id=$1`, [id]);
  return rows[0] || null;
}
...
async function findByAuthor(authorId) {
  return (
    await db.query(
      `${SELECT_POST} WHERE p.author_id=$1 ORDER BY p.created_at DESC`,
      [authorId],
    )
  ).rows;
}
